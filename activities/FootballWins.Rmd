...
Based on what I've seen, this is a useful model because it points out any variation in the data. In specific, the variation is 62% whcich means that there were patterns in the data and it wasn't just due to random chance. The data helps to clarify why certain teams won more than other teams. In conclusion, the model is effective in pointing out trends in the data, but it's ineffective to make predictions since there is a prediction error, making the wins be off by an average of 2 wins.
...
I believe that Points Scored has the most impact on wins, as well as Takeaways. However, Yards Gained does not seem impactful at all. Points Scored gave the conclusion that the teams that win more games are the ones that tend to score more points. Takeaways also make effective connections between the data but it's not as reliable as Points Scored. Yards Gained has no impact because the effect is small and not statistically significant.

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

This model could be used to cover a team or a game in the sense that it highlights which specific performance from a team led to more wins. Points Scored and Takeaways are two  factors that could be used to explain why a team is performing a certain way, whether that's winning or losing. It could emphasize a team's strengths and weaknesses in order to foresee how the rest of the season would play out.
