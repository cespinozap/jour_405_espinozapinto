...
As the number of students in the FARMS percentage increases, the percentage of students meeting or exceeding math expectations decreases. The decreasing trend suggests that there's a negative relationship between FARMS percentage and percentage of students that met or exceeded math expectations. Schools with more students in the FARMS percentage are not meeting math expectations. This gives the impression that students in low income relatively have lower math rates than the rest of the population.
...
This model is not worth using because the FARMS percentage and math scores do not have a relationship. The p-value is not statistically significant and the adjusted R-squared values are low, so economic status and performance on math scores are connected. But on their own, economic status and math scores can't predict performance.
...
The largest differences between the predicted result and the actual math scores were 31 points above the expected and 35 points below the expected. The model isn't accurately displaying that some schools are doing better or worse thatn the displayed data.
...
Eventhough the model shows that students on free and reduced price means score a bit lower, this model can't support that there's a real connection between whether or not students with free lunch score worse on math scores than students that pay for lunch.

I believe that the school board member is wrong and there would have to be tests conducted to prove his theory right. But even then, it's ethically wrong because student that receive free lunch are just as capable as other students with more opportunities to meet the math score standards. 
