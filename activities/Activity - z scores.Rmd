...
There is no trend in the time series of daily calls. There is no real pattern as the number vary every day. There can be a day with a spike in calls and a day with little to no calls.
...
The unusally high count is 11 and the unusually low count is 7, this differs from my answer on task 3 because the data varies so much that it wasn't clear where most of the data lied. This data differed from task 3 because adding the z-scores made the unusually high and unusually low day counts stand out.
...
I would describe the day with the highest score as a day in which the amount of calls received were higher than what was expected. For example comparing this week's Tuesday's call counts vs. last week's Tuesday call counts.
...
Based on the z-score analysis it's clear that most days have an average amount of calls. But, on the days in which there is an unusually high amount of calls, the spike could be influenced by an outside source. Since the calls being received are for overdose, a spike in calls could give the impression that there's a fatal drug on the street, in order to warn people. The days that are unusually high are more worth researching than the days that are unsually low. 
